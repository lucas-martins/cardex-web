...
export function CollectionDetailsPage() {
  ...
  const [checklist, setChecklist] =
    useState<CollectionChecklist | null>(null);

  const [filter, setFilter] =
    useState<ChecklistFilter>("ALL");

  const [sectionFilter, setSectionFilter] =
    useState<CardSectionFilter>("ALL");
  ...
  const [sort, setSort] =
    useState<ChecklistSort>("CARD_NUMBER");
  ...
  const [addingWishlistId, setAddingWishlistId] =
    useState<string | null>(null);

  const [
    updatingWishlistPriorityId,
    setUpdatingWishlistPriorityId,
  ...
  const visibleCards =
    [...filteredCards].sort(
      (
        ...
        if (sort === "NAME_DESC") {
          return secondCard.name.localeCompare(
            firstCard.name,
          );
        }
      ...
      setChecklist(
        (currentChecklist) => {
          ...
          return {
            ...currentChecklist,
            cards:
              currentChecklist.cards.map(
                (currentCard) =>
                  currentCard.externalId ===
                  card.externalId
                    ? {
                        ...currentCard,
                        inWishlist:
  ...
  async function handleRemoveFromWishlist(
    ...
    try {
      ...
      toast.success(
        `${card.name} was removed from your wishlist.`,
      );
    } catch {
      toast.error(
  ...
  async function handleAddedToCollection(
    card: CollectionChecklistCard,
  ) {
    if (!collectionId) {
      return;
  ...
  async function handleWishlistPriorityChange(
    ...
    try {
      ...
      const updatedWishlistCard =
        await updateWishlistPriority(
          card.wishlistId,
          {
            priority,
          },
      ...
      toast.success(
        `${card.name} wishlist priority was updated.`,
      );
    } catch {
      toast.error(
  ...
  return (
    <section className="collection-details-page">
      ...
      <header className="collection-details-header">
        <div>
          <span>
            {checklist.collectionId}
          </span>

          <h1>
            {checklist.collectionName}
          </h1>

          <p>
            {checklist.ownedUniqueCards} /{" "}
            {checklist.totalCards} cards
            collected
          </p>
        </div>

        <strong>
          {checklist.completionPercentage.toFixed(
            2,
          )}
          %
        </strong>
      ...
      <div className="collection-details-toolbar">
        <div className="collection-details-search">
          <label htmlFor="collection-card-search">
            Search cards
          </label>
        ...
        <div className="collection-details-rarity">
          <label htmlFor="collection-card-rarity">
            Rarity
          </label>
        ...
        <div className="collection-details-sort">
          <label htmlFor="collection-card-sort">
            Sort by
          </label>
      ...
      <div className="collection-checklist-filter-groups">
        <div className="collection-checklist-filter-group">
          <span className="collection-checklist-filter-label">
            Status
          </span>
